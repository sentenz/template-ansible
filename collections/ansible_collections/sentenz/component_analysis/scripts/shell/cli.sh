...
# Arguments:
#   $1 - format (table,json,template,sarif,cyclonedx,spdx,spdx-json,github,cosign-vuln)
#   $2 - image
#   $3 - logfile
# Returns:
#   $? - Result
#########################
function cli_trivy_sbom_filesystem() {
  local format="${1:?format is missing}"
  local image="${2:?path is missing}"
  local log="${3:?log is missing}"

  util_dir_create "${log}"

  trivy --quiet filesystem --no-progress --format "${format}" --output "${log}" "${path}"
}
...
function cli_trivy_sbom_image() {
  ...
  trivy --quiet image --quiet --no-progress --format "${format}" --output "${log}" "${image}"
}
...
function cli_trivy_repo() {
  ...
  trivy --quiet repo --quiet --scanners vuln,misconfig,secret,license --format "${format}" --output "${log}" "${path}"
}
